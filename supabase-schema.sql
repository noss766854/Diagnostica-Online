  ...
  (value - 'geminiEndpoint' - 'geminiModel') || jsonb_build_object(
    ...
    'privacyText', case
      when coalesce(value->>'privacyText', '') = '' or value->>'privacyText' like 'We collect account information, saved conversations%'
        then 'We collect account details; vehicle information such as VIN or ECU identifiers when supplied; symptoms, DTCs, messages, uploads, AI usage and token estimates; booking/payment identifiers; consent choices; and technical security logs. We use this data to provide and secure the service, enforce plan limits, send account or booking emails, process payments, show ads on free plans after consent, and improve diagnostics. Data may be processed by Supabase, the configured AI provider, Resend, Stripe, Jitsi, and Google AdSense. Google may use cookies, local storage, IP addresses, device information, and ad interactions to provide, measure, and personalize ads according to your consent choices and Google policies. Contact the listed support address for access or deletion requests, subject to legal and fraud-prevention retention duties.'
